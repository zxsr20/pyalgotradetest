...
#         self.hisDs = self.macdDs.getHistogram()
        ...
        self.tds = dataseries.SequenceDataSeries(100000)
        self.w = 4
        self.jizhi = 0.0
        self.hls = []
    def gettds(self):
    ...
    def onBars(self, bars):
        ...
        if len(self.__longSMA) < 663:
            self.tds.appendWithDateTime(bar.getDateTime(),self.__longSMA[-1])
            return

        print self.__longSMA[-3]
        print self.__longSMA[-2]
        print self.__longSMA[-1]
        if self.jizhi == 0:
            if  (self.__longSMA[-2]-self.__longSMA[-3])*(self.__longSMA[-2]-self.__longSMA[-1]) > 0:#-2是极值
                self.jizhi = self.__longSMA[-2]
                
                hl = CHLTicket()
                hl.id = len(self.hls)
                hl.idx = len(self.__priceDS) - 1
                hl.price = self.__longSMA[-2]
                if self.__longSMA[-2]-self.__longSMA[-3] > 0:
                    hl.hl = 1
                else:
                    hl.hl = 2
                self.hls.append(hl)
                
                if abs(self.__longSMA[-1] - self.jizhi) <= self.w:
                    self.tds.appendWithDateTime(bar.getDateTime(),self.jizhi)
...
#         myStrategy4.getticketidx().append(len(data_lines)-1)
    feed = yahoofeed.Feed()
    feed.addBarsFromCSV(instrumentID, date+instrumentID+".csv")
    myStrategy1 = MyStrategyRSI1(feed,instrumentID, 30,20,100,57,45,appconsant.waveseg_nwaveticket,appconsant.waveseg_y,appconsant.waveseg_x,appconsant.waveseg_rongren)
    ...
    plt.getInstrumentSubplot("sma").addDataSeries("longsma", myStrategy1.getlongSMA())
    plt.getOrCreateSubplot("tds").addDataSeries("tds", myStrategy1.gettds())

...
#         self.__price_index = 0
        ...
        self.spanDS =  dataseries.SequenceDataSeries(100000)#use to draw map
...
#         print str(pds)
        ...
        self.spanDS.appendWithDateTime(bar.getDateTime(),rangeUpper-rangeLower)
        ...
        if len(self.t_ranges) < self.t_rangeslen:
            self.t_ranges.append(rangeUpper-rangeLower)
            self.t_rangeprices.append(self.__priceDS[-1])
        else:
            self.t_ranges.remove(self.t_ranges[0])
            self.t_ranges.append(rangeUpper-rangeLower)
            self.t_rangeprices.remove(self.t_rangeprices[0])
            self.t_rangeprices.append(self.__priceDS[-1])
        ...
        if self.__longPos is not None or self.__shortPos is not None:#已入场
            ...
            if (rangeUpper-rangeLower) - min(self.t_ranges) > 20:
                if self.t_rangeprices[-1] == max(self.t_rangeprices) or self.t_rangeprices[-2] == max(self.t_rangeprices):
    #                 printLog('当前价格大于范围价格加y'+str(self.rangespan_y)+'，准备做多')
                    shares = 1
                     
                    printLog('做多之前我的钱'+str(self.getBroker().getCash()))
                    self.__longPos = self.enterLong(self.__instrument, shares, True)
                elif self.t_rangeprices[-1] == min(self.t_rangeprices) or self.t_rangeprices[-2] == min(self.t_rangeprices):
                    shares = 1
    #                     
...
#         if (rangeUpper - rangeLower) < self.smallrange:
#             self.smallrange = (rangeUpper - rangeLower)
#             self.rangelowertime+=1
#         else:
#             self.rangelowertime-=1
#         if self.rangelowertime > 50 :
            
            
            
#         if self.lastpricerange ==0.0:
#             self.lastpricerange = rangeUpper - rangeLower 
#         if (rangeUpper - rangeLower) < self.lastpricerange:
#             self.lastpricerange = (rangeUpper - rangeLower)
#         elif (rangeUpper - rangeLower) - self.lastpricerange > 20:
...
#     plt.getOrCreateSubplot("wave").addDataSeries("waveds", myStrategy.getWave())
    ...
    plt.getOrCreateSubplot("range").addDataSeries("range", myStrategy.getrange())

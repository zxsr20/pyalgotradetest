...
def worker_process(strategyClass, port,celuename,celuedate):
...
#     print 'worker_process celue'+celuename
    w = Worker("localhost", port, name,celuename,celuedate)
...
def run(strategyClass, barFeed, strategyParameters, workerCount=None):
    ...
    serverThread.start()
    print 'local run celue'+pyalgotrade.logger.celue

    try:
        # Build the worker processes.
        for i in range(workerCount):
            workers.append(multiprocessing.Process(target=worker_process, args=(strategyClass, port,pyalgotrade.logger.celue,pyalgotrade.logger.celuedate)))

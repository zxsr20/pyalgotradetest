...
#     ret = datetime.datetime(year, month, day)
    ...
    microsecond = int(date[18:22])*1000
...
#         if self.__dailyBarTime is not None:toting
#             ret = datetime.datetime.combine(ret, self.__dailyBarTime)
    ...
    def parseBar(self, csvRowDict):
        ...
        bidvol = float(csvRowDict["bidvol"])
        askvol = float(csvRowDict["askvol"])

        if self.__sanitize:
            open_, high, low, close = common.sanitize_ohlc(open_, high, low, close)

        return bar.BasicBar(dateTime, open_, high, low, close, volume, adjClose,bidvol,askvol, self.__frequency)
...
class Feed(csvfeed.BarFeed):
    ...
    def __init__(self, frequency=bar.Frequency.SECOND, timezone=None, maxLen=dataseries.DEFAULT_MAX_LEN):
        if isinstance(timezone, int):
            raise Exception("timezone as an int parameter is not supported anymore. Please use a pytz timezone instead.")

#         if frequency not in [bar.Frequency.DAY, bar.Frequency.WEEK]:#toting

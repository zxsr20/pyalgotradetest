...
#             if self.getValue(dateTime) == self.lastprice:
#                 dtimes.append(dateTime)
#             else:
#                 values.append(self.getValue(dateTime))
#                 self.lastprice = self.getValue(dateTime)
#         for tt in dtimes:
#             dateTimes.remove(tt)
...
#             print series
    ...
    def getSeries(self, name, defaultClass=LineMarker):
        try:
            ...
            #for some special name serie give them special color
            if 'cancel' in name:
                ret = CancelMarker()
            elif 'buyping' in name:
                ret = BuyPingMarker() 
            elif 'sellping' in name:
                ret = SellPingMarker() 
            elif 'buy' in name:
                ret = BuyMarker() 
            elif 'sell' in name:
...
#         print 'InstrumentSubplot onBars'
        if bar:
            dateTime = bars.getDateTime()
...
class StrategyPlotter(object):
    ...
    def __init__(self, strat, plotAllInstruments=True, plotBuySell=True, plotPortfolio=False,plotOrder=False):
...
#         print 'StrategyPlotter __onBarsProcessed'
        ...
        # Feed the portfolio evolution subplot.toting
...
#             self.__portfolioSubplot.onBars(bars)
    ...
    def plot(self, fromDateTime=None, toDateTime=None):
        ...
        plt.title('rangespan_ma='+str(appconsant.rangespan_ma)+' rangespan_range='+str(appconsant.rangespan_range)+' rangespan_span='+str(appconsant.rangespan_span), fontdict=font)
        
        plt.show()
        
    def plotdiffbar(self, dataSeries,width=0.5):
        """Plots the strategy execution. Must be called after running the strategy.
        ...
        plt.bar(dataSeries.getDateTimes(), dataSeries.getValues(), width,alpha = .5, color = 'g')

        plt.show()
         
    def plotdiff2line(self, dataSeries1,dataSeries2,width=0.01):
        """Plots the strategy execution. Must be called after running the strategy.
...
#         plt.bar(dataSeries.getDateTimes(), dataSeries.getValues(), width,alpha = .5, color = 'g')
#         values = []
#         for dateTime in dataSeries:
#             values.append(self.getValue(dateTime))
        plt.plot(dataSeries1.getDateTimes(), dataSeries1.getValues(), color='g')
        plt.plot(dataSeries2.getDateTimes(), dataSeries2.getValues(), color='r')
        plt.show()
        
    def plotdiffbarbytwo(self, dataSeries1,dataSeries2):
        plt.bar(dataSeries1, dataSeries2,0.01,color = 'g')
        plt.bar(dataSeries1, dataSeries2,0.001,color = 'r')
        plt.show()
        
    def plotdiffline(self, dataSeries,width=0.01):
        """Plots the strategy execution. Must be called after running the strategy.
...
#         for dateTime in dataSeries:
#             values.append(self.getValue(dateTime))
        plt.plot(dataSeries.getDateTimes(), dataSeries.getValues(), color='g')
        plt.show()

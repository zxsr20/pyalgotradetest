...
#         logLogger = logging.getLogger('log')
        if order.isBuy():
            price = fillInfo.getaskprice()
            self.printLog(order.getInstrument()+'价格getaskprice:'+str(price)+'同时getbidPrice'+str(fillInfo.getbidPrice())+'同时avgPrice'+str(fillInfo.getPrice()))
            quantity = fillInfo.getQuantity()
            
            cost = price * quantity * -1
            self.printLog(order.getInstrument()+'数量:'+str(quantity)+'花费:'+str(abs(cost))+'时间:'+str(dateTime))
            assert(cost < 0)
            sharesDelta = quantity
        elif order.isSell():
            price = fillInfo.getbidPrice()
            self.printLog(order.getInstrument()+'价格getbidPrice:'+str(price)+'同时getaskprice'+str(fillInfo.getaskprice())+'同时avgPrice'+str(fillInfo.getPrice()))
            quantity = fillInfo.getQuantity()
...
#         print 'resultingCash:'+str(resultingCash)
...
#             print 'ting resultingCash:'+str(self.__cash)
#             print 'updatedShares:'+str(updatedShares)
            if updatedShares == 0:

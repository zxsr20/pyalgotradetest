...
def get_limit_price_trigger(action, limitPrice, useAdjustedValues, bar):
    ...
    close_ = bar.getClose(useAdjustedValues)
...
class FillInfo(object):
    def __init__(self, price, quantity,bidprice,askprice):
        self.__price = price
    ...
    def onBars(self, broker_, bars):
        ...
        for instrument in bars.getInstruments():
            ...
            if bar.getFrequency() == pyalgotrade.bar.Frequency.TRADE:
                ...
                print 'onBars'+str(bar.getPrice())
                print 'onBars'+str(bar.getVolume())
...
#         else:
        price = bar.getAdjClose(broker_.getUseAdjustedValues())
            
        bidprice = bar.getBid1(broker_.getUseAdjustedValues())
        askprice = bar.getAsk1(broker_.getUseAdjustedValues())
        ...
        return FillInfo(price, fillSize,bidprice,askprice)
    ...
    def fillLimitOrder(self, broker_, order, bar):
        ...
        if price is not None:
            ret = FillInfo(price, fillSize,price,price)
        return ret

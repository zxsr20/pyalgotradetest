...
class DMD:
    def __init__(self):
        ...
        self.info = ''
        self.date_format = ''
        self.ask1_format = 0.0
        self.bid1_format = 0.0
        self.avg_format = 0.0
    def toString(self):     
        return 'Volume:'+str(self.Volume)+'ClosePrice:'+str(self.ClosePrice)+'BidPrice1:'+str(self.BidPrice1)+'Turnover:'+str(self.Turnover)
    def getbid1(self):
...
def getDayData(InstrumentID,path): #此处yh修改
    ...
    for i in range(0,nCnt):
        ...
        dmd.InstrumentID = InstrumentID#sys.argv[2] #dmd.InstrumentID.lstrip("\0")  #将取出的数据封装成dmd类型
        #print s_r
...
if __name__=="__main__":
    ...
    l = getDayData('20150518','IF1506')   
    print len(l)
   
    writer = csv.writer(file('your6.csv', 'wb'))
...
#     for i in l : 
# #         print  str(datetime.datetime.strptime(i.TradingDay, "%Y-%m-%d"))
# #         year = int(i.TradingDay[0:4])
# #         month = int(i.TradingDay[5:7])
# #         day = int(i.TradingDay[8:10])
#         print i.TradingDay[0:4]+'/'+i.TradingDay[4:6]+'/'+i.TradingDay[6:8]+' '+str(i.UpdateTime)
#         writer.writerow([i.TradingDay[0:4]+'/'+i.TradingDay[4:6]+'/'+i.TradingDay[6:8]+' '+str(i.UpdateTime),str(i.LastPrice),str(i.LastPrice),str(i.LastPrice),str(i.LastPrice),str(i.Volume),str(i.LastPrice)])
    ...
    c=0
    for i in l : 
        if a==str(i.TradingDay)+' '+str(i.UpdateTime)[0:8]+'.'+str(i.UpdateMillisec) or str(i.UpdateTime)[0:8] < '09:00:00':
#             print str(i.TradingDay)+' '+str(i.UpdateTime)[0:8]+'.'+str(i.UpdateMillisec)
            ...
            writer.writerow([str(i.TradingDay)+' '+str(i.UpdateTime)[0:8]+'.'+str(i.UpdateMillisec),str(i.LastPrice),str(i.LastPrice),str(i.LastPrice),str(i.LastPrice),str(i.Volume),str(i.LastPrice)])
            b=i
        a=str(i.TradingDay)+' '+str(i.UpdateTime)[0:8]+'.'+str(i.UpdateMillisec) 
#         print " ".join([str(i.InstrumentID),str(i.TradingDay),str(i.UpdateTime)[0:8],str(i.UpdateMillisec),str(i.LastPrice),str(i.Volume),str(i.AskPrice1),str(i.AskVolume1),str(i.BidPrice1),str(i.BidVolume1)])
    print 'finish'
#     writer.writerow

...
class MyStrategy4LeaveByWaveZ(strategy.BacktestingStrategy):
    def __init__(self, feed,instrument, longSMA, shortSMA, rsiPeriod, overBoughtThreshold, overSoldThreshold,nwaveticket,waveseg_y,waveseg_x,stoploss):
    ...
    def onBars(self, bars):
        ...
        if self.__longPos is not None or self.__shortPos is not None:
            if self.__longPos is not None:
                ...
                if self.buyprice - self.__priceDS[-1] >= self.stoploss:
                    printLog('当前亏损大于止损点,准备离场buyprice'+str(self.buyprice)+' -price'+str(self.__priceDS[-1])+' >=stoploss'+str(self.stoploss))
                    self.__longPos.exitMarket()
                    self.buyprice = 0
                    printLog('做多平仓')
                elif self.wavetickets[-1].price_wave <= self.waveseg_x:
                    printLog('当前波动小于x'+str(self.waveseg_x))
                    self.__longPos.exitMarket()
                    self.buyprice = 0
                    printLog('做多平仓')
            if self.__shortPos is not None:
                if self.buyprice == 0:
                    self.buyprice = self.__priceDS[-1]
                if self.__priceDS[-1] - self.buyprice >= self.stoploss:
                    printLog('当前亏损大于止损点,准备离场price'+str(self.__priceDS[-1])+' -buyprice'+str(self.buyprice)+' >=stoploss'+str(self.stoploss))
...
#     logfile = appconsant.logfile
    ...
    myStrategy = MyStrategy4LeaveByWaveZ(feed,instrumentID, 660,200,100,57,45,appconsant.leavebywavez_nwaveticket,appconsant.leavebywavez_y,appconsant.leavebywavez_x,appconsant.leavebywavez_stoploss)

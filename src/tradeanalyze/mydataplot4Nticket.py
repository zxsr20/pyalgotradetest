...
class MyStrategyRSI(strategy.BacktestingStrategy):
    def __init__(self, feed,instrument, longSMA, shortSMA, rsiPeriod, overBoughtThreshold, overSoldThreshold,datalines):
...
#     def generateSegPankou(self,datetime,price):
    ...
    def onBars(self, bars):
        ...
        for i in range(appconsant.nticket):
            line = self.__datalines[self.i_datalines]
            print >> self.logfile," ".join([str(line.InstrumentID),str(line.TradingDay),str(line.UpdateTime)[0:8],str(line.UpdateMillisec),str(line.LastPrice),str(line.Volume),str(line.AskPrice1),str(line.AskVolume1),str(line.BidPrice1),str(line.BidVolume1)])
            self.i_datalines += 1
        ...
        if len(self.__priceDS) == 2:#the second point
            ...
            if self.__priceDS[-2] > self.__priceDS[-1]:
                ...
                self.allSegPankous1[-1].dir = 2 #up
#             if self.__priceDS[-2] == self.__priceDS[-1]:
...
#     feed.addBarsFromCSV("rb1510", "20150708rb1510.csv")
    ...
    myStrategy = MyStrategyRSI(feed,instrumentID, 660,200,100,57,45,data_lines)

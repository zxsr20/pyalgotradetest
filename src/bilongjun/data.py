...
def getDayData(Date,InstrumentID,path=""): #此处yh修改 date为某一天的日期
    fz= zipfile.ZipFile("D:\pyTest_yh\data\\"+Date+".zip","r")
...
if __name__=="__main__":
    ...
    l = getDayData('20141125','IF1412',path)   
#     for i in l : 
#         print " ".join([str(i.InstrumentID),str(i.TradingDay),str(i.UpdateTime)[0:8],str(i.UpdateMillisec),str(i.LastPrice),str(i.Volume),str(i.AskPrice1),str(i.AskVolume1),str(i.BidPrice1),str(i.BidVolume1)])
    ...
    writer = csv.writer(file('your3.csv', 'wb'))
     
    writer.writerow(['Date', 'Open', 'High','Low','Close','Volume','Adj Close'])
    a=''
    for i in l : 
        if a==str(i.TradingDay)+' '+str(i.UpdateTime)[0:8]+'.'+str(i.UpdateMillisec):
